...
namespace Cognite.Extractor.StateStorage
{
    ...
    public class BaseExtractionState : IExtractionState
    {
        ...
        public BaseExtractionState(string id, bool frontfill = false, bool backfill = false)
        {
            Id = id;
        ...
        public virtual void FinalizeRangeInit()
        {
            ...
            lock (_mutex)
            {
                if (DestinationExtractedRange.First == CogniteTime.DateTimeEpoch && DestinationExtractedRange.Last == DateTime.MaxValue)
                {
