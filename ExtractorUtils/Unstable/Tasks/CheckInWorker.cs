        ...
        private async Task TryWriteCheckIn(IEnumerable<ErrorWithTask> errors, IEnumerable<TaskUpdate> tasks, IEnumerable<ActionUpdate> actionUpdates, CancellationToken token)
        {
            ...
            catch (Exception ex)
            {
                if (ex is ResponseException rex && (rex.Code == 400 || rex.Code == 404))
                {
                    ...
                    // update for is unknown to the server. When the response identifies the
                    // specific bad externalId(s) via the standard "missing" error field, drop
                    // only those action updates and requeue everything else -- errors, task
                    // updates, and any other action updates in the batch are not at fault and
                    // shouldn't be lost just because one entry was rejected.
