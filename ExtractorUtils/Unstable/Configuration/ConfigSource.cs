    ...
    public class LocalConfigSource<T> : ConfigSource<T> where T : VersionedConfig
    {
        ...
        public override async Task<bool> ResolveConfig(int? targetRevision, BaseErrorReporter reporter, CancellationToken token)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    reporter.Fatal($"Fatally failed to load local configuration file from {_configFilePath}: {ex.Message}", type: "config");
                }
                _lastErrorMsg = msg;
                throw;
            }
    ...
    public class RemoteConfigSource<T> : ConfigSource<T> where T : VersionedConfig
    {
        ...
        public override async Task<bool> ResolveConfig(int? targetRevision, BaseErrorReporter reporter, CancellationToken token)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (_lastErrorMsg != msg)
                {
                    reporter.Fatal($"Fatally failed to load configuration file from CDF: {msg}", type: "config", configRevision: targetRevision);
                }
                _lastErrorMsg = msg;
                throw;
            }
            ...
            catch (Exception ex)
            {
                if (isNewConfig)
                {
                    reporter.Fatal($"Failed to parse configuration file from CDF: {ex.Message}", type: "config", configRevision: revision);
                }
                throw;
            }
            return true;
