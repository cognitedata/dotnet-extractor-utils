    ...
    public class LocalConfigSource<T> : ConfigSource<T> where T : VersionedConfig
    {
        ...
        public override async Task<bool> ResolveConfig(int? targetRevision, BaseErrorReporter reporter, CancellationToken token)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    reporter.Fatal($"Fatally failed to load local configuration file from {_configFilePath}: {ex.Message}");
                }
                _lastErrorMsg = msg;
                throw;
            }
    ...
    public class RemoteConfigSource<T> : ConfigSource<T> where T : VersionedConfig
    {
        ...
        public override async Task<bool> ResolveConfig(int? targetRevision, BaseErrorReporter reporter, CancellationToken token)
        {
            ...
            catch (Exception ex)
            {
                if (isNewConfig)
                {
                    reporter.Fatal($"Failed to parse configuration file from CDF: {ex.Message}", configRevision: revision);
                }
                throw;
            }
            return true;
