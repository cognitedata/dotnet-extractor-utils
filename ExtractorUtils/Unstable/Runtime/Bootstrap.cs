...
namespace Cognite.Extractor.Utils.Unstable.Runtime
{
    ...
    {
        private List<ExtractorError> _pendingErrors { get; } = new List<ExtractorError>();
                ...
                if (_client != null && _integrationId != null)
                {
                    await _client.Alpha.Integrations
                        .CheckInAsync(new CheckInRequest
                        {
                            ...
                            Errors = _pendingErrors.Select(e => new ErrorWithTask
                            {
                                Level = e.Level,
                                Description = e.Description,
                                Details = e.Details,
                                StartTime = e.StartTime.ToUnixTimeMilliseconds(),
                                EndTime = e.EndTime?.ToUnixTimeMilliseconds()
                            }).ToList()
