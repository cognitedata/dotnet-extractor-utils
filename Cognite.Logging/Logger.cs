...
namespace Cognite.Extractor.Logging
{
    ...
    public static class LoggingUtils
    {
        ...
        public static Serilog.ILogger GetConfiguredLogger(LoggerConfig config, LogEventLevel baseMinLevel)
        {
            ...
            GetSerilogDefault().Information("Configuring Logger with level output to Console: {Console}, File: {File}, Stderr: {Stderr}",
                config.Console?.Level, config.File?.Level, config.Console?.StderrLevel);
            return logConfig.CreateLogger();
        }
